...
function buildCompactCoachingPdf(data,{includeNames}){
  ...
  return `<section class="pdf-ai-section"><section class="pdf-block pdf-coaching-page-heading" data-pdf-page-start><h2>학생 코칭</h2><p>저장된 학생 설문 기준의 대화 초안입니다. 전체 이력을 새로 분석한 결과는 아닙니다.</p></section>${block('함께 탐색할 주제',short(result.summary?.text),result.summary?.refs)}${block('대화를 여는 질문',short(result.question?.text),result.question?.refs)}${(result.actions||[]).slice(0,3).map(action=>block(`답에 따라 이어갈 대화: ${action.title}`,(action.steps||[]).slice(0,2).map(short).join('<br>'),action.refs)).join('')}${block('원한다면, 작은 시도와 돌아보기',short(result.check_after).replace(/\r?\n/g,'<br>'))}</section>`;
}
