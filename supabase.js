...
async function teacherRpc(functionName,payload={}){const session=getTeacherSession();if(!session?.access_token)throw new Error('교사 로그인이 필요합니다.');const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/${functionName}`,{method:'POST',headers:{apikey:SUPABASE_PUBLISHABLE_KEY,Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json().catch(()=>null);if(response.status===401){setTeacherSession(null);throw new Error('로그인 시간이 만료되었습니다. 다시 로그인해 주세요.')}if(!response.ok)throw new Error(result?.message||'권한이 있는 학급인지 확인해 주세요.');return result}
async function teacherEdgeFunction(functionName,payload={}){const session=getTeacherSession();if(!session?.access_token)throw new Error('교사 로그인이 필요합니다.');const response=await fetch(`${SUPABASE_URL}/functions/v1/${functionName}`,{method:'POST',headers:{apikey:SUPABASE_PUBLISHABLE_KEY,Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json().catch(()=>null);if(response.status===401){setTeacherSession(null);throw new Error('로그인 시간이 만료되었습니다. 다시 로그인해 주세요.')}if(!response.ok)throw new Error(result?.error||'서버 분석 요청에 실패했습니다.');return result}
