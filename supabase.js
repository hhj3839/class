...
async function teacherSignUp(email,password){const response=await fetch(`${SUPABASE_URL}/auth/v1/signup`,{method:'POST',headers:{apikey:SUPABASE_PUBLISHABLE_KEY,'Content-Type':'application/json'},body:JSON.stringify({email,password})});const result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.msg||result?.error_description||'회원가입에 실패했습니다.');if(result?.access_token)setTeacherSession(result);return result}
