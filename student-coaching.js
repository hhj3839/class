...
function renderStudentCoachingCard(){
  ...
  target.innerHTML=`<div class="panel-head"><div><p class="eyebrow">선택한 학생을 위한 지도 초안</p><h3>학생 코칭 카드</h3></div>${card?'<span class="confidence">교사 검토 필요</span>':''}</div>
    <p class="muted">자료 기준 ${escape(data.basisMonth?monthLabel(data.basisMonth):'자료 없음')} · 최근 12개월 안의 응답·관찰 참고 · 생성 요청 잔여 ${Number(data.remaining)||0}회</p>
    ...
    <div class="coaching-buttons"><button type="button" class="primary-button" data-coaching-generate ${state.generating||!data.canGenerate||data.remaining<=0?'disabled':''}>${state.generating?'AI 코칭 생성 중…':card?'적용 결과를 참고해 다시 만들기':data.stale?'현재 자료로 다시 만들기':'코칭 카드 만들기'}</button>${card?'<button type="button" class="text-button" data-coaching-delete>카드 삭제</button>':''}</div><p class="muted coaching-usage">새 생성은 외부 AI 비용이 발생합니다. 학생 코칭 생성 요청은 학급당 월 10회(실패한 요청 포함)이며, 저장 카드 조회는 차감하지 않습니다. 학생 유형·성격 진단이 아닌 교사용 초안입니다.</p>`;
}
...
document.addEventListener('click',async event=>{
  ...
  if(generate&&!confirm('선택한 학생의 응답과 교사 기록에서 등록된 학생 이름·이메일·휴대전화번호를 가린 뒤 외부 AI로 전송할까요? 다른 개인정보는 기록에 포함하지 마세요. 생성 요청 1회가 차감됩니다.'))return;
