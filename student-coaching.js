...
function renderStudentCoachingCard(){
  ...
  target.innerHTML=`<div class="panel-head relationship-ai-head coaching-ai-head"><h3>학생 코칭 카드</h3><div class="ai-review-actions"><button type="button" class="text-button" data-coaching-reload ${busy?'disabled':''}>${state.loading?'불러오는 중…':'저장된 결과 불러오기'}</button><button type="button" class="text-button" data-coaching-generate ${busy||!data.canGenerate||data.remaining<=0?'disabled':''}>${state.generating?'AI 분석 중…':'AI 새 분석'}</button></div></div>
    ...
    ${error}${data.stale?'<div class="notice warning"><p>자료 또는 분석 기준이 변경되어 이전 카드를 숨겼습니다. 학생 설문만 사용하는 기준으로 다시 생성해 주세요.</p></div>':''}
    ${data.limited?'<p class="coaching-limited">자료가 적습니다. 해석보다 확인 질문을 중심으로 참고하세요.</p>':''}
    ${result?`<section class="coaching-summary"><h4>이번 코칭의 초점</h4>${item(result.summary)}</section>
    <section class="coaching-question"><h4>학생에게 건넬 말</h4><blockquote>${escape(aiTeacherDisplayText(result.question.text))}</blockquote>${refs(result.question.refs)}</section>
    <section class="coaching-action-section"><h4>교사가 해볼 일</h4><div class="coaching-actions-grid">${result.actions.map((action,index)=>`<article class="coaching-action"><h5><span>${index+1}</span>${escape(aiTeacherDisplayText(action.title))}</h5><ol>${action.steps.map(step=>`<li>${escape(aiTeacherDisplayText(step))}</li>`).join('')}</ol>${refs(action.refs)}</article>`).join('')}</div></section>
    ...
    `<div class="coaching-empty"><h4>${data.canGenerate?'아직 현재 자료의 코칭 카드가 없습니다.':'코칭을 만들 근거가 부족합니다.'}</h4><p>${data.canGenerate?'상단의 AI 새 분석을 누르면 응답 근거에 맞춘 지도 초안을 생성합니다.':'학생이 설문을 제출한 뒤 확인해 주세요.'}</p></div>`}`;
}
