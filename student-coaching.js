...
function coachingDialogueMarkup(value){
  ...
  let result='',offset=0,matched=false;
  for(const match of text.matchAll(pattern)){matched=true;result+=escapeHTML(text.slice(offset,match.index))+`<strong class="coaching-spoken">${escapeHTML(match[0])}</strong>`;offset=match.index+match[0].length}
  if(!matched&&/[?？]\s*$/.test(text)&&!/[.!。]\s+/.test(text))return `<strong class="coaching-spoken">${escapeHTML(text)}</strong>`;
...
function renderStudentCoachingCard(){
  ...
  target.innerHTML=`<div class="panel-head relationship-ai-head coaching-ai-head"><h3>학생 코칭 카드</h3><div class="coaching-analysis-toolbar"><p class="coaching-last-analysis" role="status">${escape(coachingLastAnalysisMeta(data))}</p><div class="ai-review-actions"><button type="button" class="text-button" data-coaching-reload ${busy?'disabled':''}>${state.loading?'불러오는 중…':'저장된 결과 불러오기'}</button><button type="button" class="text-button" data-coaching-generate ${busy||!data.canGenerate||data.remaining<=0?'disabled':''}>${state.generating?'AI 분석 중…':'AI 새 분석'}</button></div></div></div>
    ...
    <details class="coaching-disclosure coaching-info"><summary>분석 범위·전송 안내</summary><p class="coaching-scope">최근 12개월 내 본인의 최신 4회 설문 중 고민·도움 요청·놀림·폭력 관련 경험·도움이 필요한 친구·자기평가 등, 본인 응답이 있는 달의 받은 관계 점수와 최신 응답 월·직전 달의 같은 학생 간 양방향 관계 관측을 참고합니다. 비교 가능한 관계만 변화 근거로 사용하며 전입일과 다른 학생의 재적 이력은 완전히 반영되지 않습니다. 모든 과거 응답·문항을 분석하는 것은 아니며, 근거별 본문은 최대 500자까지 참고합니다.</p><p>등록된 학생 이름·이메일·휴대전화번호를 가린 학생 설문 응답을 외부 AI로 전송합니다. 그 밖의 개인정보는 기록에 포함하지 마세요.</p><p>학급당 월 100회(실패한 요청 포함). 저장 결과 조회는 차감하지 않습니다. 새 분석에는 외부 AI 비용이 발생합니다.</p></details>
    ...
    ${result?`<section class="coaching-summary"><h4>함께 탐색할 주제</h4>${item(result.summary)}</section>
    <section class="coaching-question"><h4>대화를 여는 질문</h4><blockquote>${escape(aiTeacherDisplayText(result.question.text))}</blockquote>${refs(result.question.refs)}</section>
    <section class="coaching-action-section"><h4>답에 따라 이어갈 대화</h4><p class="coaching-dialogue-help">${data.safetyPriority===true?'안전 확인은 먼저 진행하고, 이후 대화는 학생 반응에 맞춰 선택하세요. 말하기를 원하지 않아도 필요한 보호는 미루지 않습니다.':'학생의 답을 듣고 필요한 질문만 골라 쓰세요. 답이나 실천 약속을 정하지 않아도 괜찮습니다.'}</p><div class="coaching-actions-grid">${result.actions.map(action=>`<article class="coaching-action"><h5>${escape(aiTeacherDisplayText(action.title))}</h5><ol>${action.steps.map(step=>`<li>${coachingDialogueMarkup(step)}</li>`).join('')}</ol>${refs(action.refs)}</article>`).join('')}</div></section>
    <section class="coaching-followup"><h4>대화 마무리</h4><p>${escape(aiTeacherDisplayText(result.check_after))}</p></section>
