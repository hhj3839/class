...
for(const width of [360,768,1440])test(`학생 코칭 생성·근거·적용 결과 ${width}px`,async({page},testInfo)=>{
  ...
  const id='11111111-1111-4111-8111-111111111111',otherId='22222222-2222-4222-8222-222222222222',students=[{number:1,name:'가상학생가',studentId:id},{number:2,name:'가상학생나',studentId:otherId}];let card=null,feedback=[],generated=0,loads=0,externalUsage=0;
  ...
  await page.route('**/*.supabase.co/**',async route=>{const url=route.request().url();expect(url).not.toMatch(/teacher_(?:get|save)_observations?_auth/);const body=route.request().postDataJSON()||{};let response=[];
    ...
    if(url.includes('/functions/v1/student-coaching')){if(body.action==='generate'){generated++;card={id:'card-1',result,generatedAt:'2026-09-09T00:00:00Z',basisMonth:'2026-09',model:'gpt-5.6-terra'}}else loads++;response={card:body.studentId===id?card:null,remaining:100-generated-externalUsage,quotaMonth:'2026-09',quotaCheckedAt:Date.now(),stale:false,canGenerate:body.studentId===id,limited:true,basisMonth:'2026-09',sources:[{id:'E1',label:'학교생활 고민',month:'2026-09',type:'학생 응답',value:'모둠에서 말하기 어려워요.'}],feedback};}
  ...
  await expect(page.locator('.coaching-followup')).toContainText('원한다면, 작은 시도와 돌아보기');await expect(page.locator('.coaching-followup>p')).toHaveCSS('white-space','pre-line');await expect(page.locator('.coaching-followup>p')).toContainText('실제로 해 본 뒤:');
  if(width===1440){
