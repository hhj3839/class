const test=require('node:test'),assert=require('node:assert/strict'),core=require('../relationship-core.js');
...
test('관계 계산 모듈은 화면과 독립적으로 상호 연결과 집단을 계산한다',()=>{const result=core.build([],[{number:1,name:'A'},{number:2,name:'B'},{number:3,name:'C'}],fakeCore);assert.equal(result.mutual.length,2);assert.deepEqual(result.groups,[[1,2]])});
