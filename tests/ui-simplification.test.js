...
test('학급 분석은 전체 현황 없이 응답 흐름·AI 학생 지원·즉시 PDF를 제공한다',()=>{
  assert.match(html,/data-analysis-tab="ai"/);
  assert.match(html,/data-analysis-tab="patterns"/);
  assert.match(html,/id="analysisAiPanel"[^>]*hidden/);
  assert.match(html,/id="printReport"/);
  assert.match(html,/우리 반 응답 흐름/);
  assert.match(html,/AI 학생 지원/);
...
test('관계 분석 결과와 관찰 포인트에서 비대칭·이번 주 단계를 제거한다',()=>{
  ...
  assert.match(actions,/AI 관계 코칭/);
  assert.match(actions,/AI 관계 코칭/);
});

test('월별·누적 관계망은 중앙 균형 배치와 별도 범례 표를 제공한다',()=>{
...
test('분석은 월 선택을 탭 옆에 두고 설정은 기본 설정과 학년 말 정리만 노출한다',()=>{
  ...
  assert.match(html,/data-settings-tab="basic"/);
  assert.match(html,/data-settings-tab="year-end"/);
  assert.doesNotMatch(html,/data-settings-tab="pilot"|data-settings-tab="governance"/);
  assert.match(html,/id="settingsPilotPanel"[^>]*hidden/);
  assert.match(html,/id="settingsGovernancePanel"[^>]*hidden/);
});

test('우리 반 응답 흐름은 기본 탭으로 두고 지난 제출 현황은 기본 접힘으로 표시한다',()=>{
  ...
  assert.match(aiRender,/renderClassObservationPatterns\(patterns\)/);
...
test('교사 홈과 학생 상세는 단순화된 탐색 구조를 사용한다',()=>{
  ...
  assert.doesNotMatch(app,/id="studentSupportTimelineSlot"/);
  assert.doesNotMatch(support,/studentSupportTimelineSlot/);
...
test('학생 상세와 관계 분석은 안전한 초기 화면을 사용한다',()=>{
  ...
  assert.match(html,/>월별 관계 지도<\/button>/);
