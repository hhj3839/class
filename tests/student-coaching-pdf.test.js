...
test('학생용 간결 PDF는 교사 적용 기록과 관찰 근거 라벨을 제외한다',()=>{const html=render({...data,sources:[{id:'E1',kind:'observation',type:'교사 관찰',label:'교사 관찰 사실',month:'2026-09'}]},{compact:true});assert.ok(html.includes('학생 코칭'));assert.ok(html.includes('함께 탐색할 주제'));assert.ok(!html.includes('교사의 적용 결과'));assert.ok(!html.includes('교사 기록'));assert.ok(!html.includes('교사 관찰 사실'))});
