...
const wordingMigration=fs.readFileSync('supabase/migrations/20260723223000_update_year_end_cleanup_messages.sql','utf8');
const prd=fs.readFileSync('PRD_v1.2.md','utf8');
const demoGuide=fs.readFileSync('DEMO_CLASS_GUIDE.md','utf8');
...
test('학년 말 자료 정리는 보고서 생성 뒤 바로 이중 확인으로 이어진다',()=>{
  for(const id of ['downloadYearEndReport','yearEndClassLabel','yearEndConfirmation','yearEndDeletionAccepted','deleteYearEndData'])assert.match(html,new RegExp(`id="${id}"`));
  assert.match(html,/학년 말 보고서 PDF 만들기/);
  assert.match(html,/전년도 학급 자료 영구 삭제/);
  assert.match(html,/현재 학년도는 삭제할 수 없습니다/);
  assert.doesNotMatch(html,/보고서 보관 확인|id="yearEndReportStored"|삭제 대상 미리보기|id="previewYearEndCleanup"|id="yearEndPreview"/);
  assert.doesNotMatch(app,/yearEndReportStored/);
...
test('삭제 RPC의 사용자 오류 문구는 화면의 2단계 흐름과 일치한다',()=>{
  assert.match(wordingMigration,/보고서를 만든 뒤 영구 삭제 준비를 다시 진행해 주세요/);
  assert.match(wordingMigration,/영구 삭제 준비 시간이 지났습니다/);
  assert.match(wordingMigration,/보고서 생성 이후 학급 자료가 변경되었습니다/);
  assert.doesNotMatch(wordingMigration,/삭제 대상 미리보기|미리보기 이후/);
});
...
test('영구 삭제는 학생 연결 자료를 트랜잭션에서 정리하고 최소 감사 기록만 다시 남긴다',()=>{
  ...
  assert.match(prd,/학년 말 보고서 PDF 생성[\s\S]*이중 입력[\s\S]*영구 삭제/);
  assert.match(prd,/별도의 보고서 보관 확인[\s\S]*표시하지 않는다/);
  assert.doesNotMatch(prd,/보고서 생성·보관 확인/);
  assert.match(demoGuide,/별도의 보관 확인이나 삭제 대상 미리보기 없이[\s\S]*바로 활성화/);
});
