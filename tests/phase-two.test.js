...
test('AI 결과에서 영문 내부 키와 확인되지 않은 직접 경험 표현을 제거한다',()=>{
  ...
  assert.match(edge,/analysisVersion='2026\.07\.21-teacher-action-focused-v6'/);
...
test('AI 분석은 마지막 분석 날짜·시간과 담임 실행 정보를 우선 표시한다',()=>{
  ...
  assert.match(app,/해석 → 관찰 → 다음 대화 순서/);
  assert.match(html,/담임 해석·관찰 포인트·다음 대화와 지원을 먼저 제시/);
});

test('AI 기본 카드는 세 단계로 줄이고 조건부 항목과 실행 개수를 제한한다',()=>{
  assert.match(edge,/teacher-action-focused-v6/);
...
test('학생 상세는 간단한 자동 분석 없이 세 탭으로 자료를 구분한다',()=>{
  assert.doesNotMatch(html,/id="studentResponseInsight"/);
  assert.doesNotMatch(app,/function studentResponseInsights\(/);
