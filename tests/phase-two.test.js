...
test('AI Edge Function은 Supabase 교사 토큰을 확인하고 익명 호출을 401로 종료한다',()=>{
  ...
  assert.match(edge,/student-support-v10/);
});
...
test('우리 반 응답 흐름은 계산 지표와 식별자를 제거한 AI 학급 요약을 표시한다',()=>{
...
test('AI 분석은 받은 평가 기반 최대 3명과 다음 대화·지원 방향을 만든다',()=>{
  ...
  assert.match(app,/<strong>응답에서 살펴볼 점<\/strong>/);
...
test('AI 분석은 전월 변화와 유형별 근거를 분리한다',()=>{
  ...
  assert.match(app,/응답에서 살펴볼 점 · 확인할 장면 · 학생 코칭 · 다음 지원 방향/);
...
test('AI 화면과 PDF는 같은 담임용 용어를 직접 사용한다',()=>{
  ...
  for(const source of [visibleAi,pdfAi]){
    ...
    assert.match(source,/담임이 확인할 장면/);
    assert.match(source,/학생 코칭/);
    assert.match(source,/다음 지원 방향/);
...
test('AI 기본 카드는 네 역할로 나누고 조건부 항목과 실행 개수를 제한한다',()=>{
  assert.match(edge,/student-support-v10/);
