...
test('AI 분석은 받은 평가 기반 최대 3명과 코칭 방향·종합 판단을 만든다',()=>{
  ...
  assert.match(app,/AI 해석 · 확인이 필요한 이유/);
  assert.match(app,/학생과 나눌 코칭 질문/);
  assert.match(app,/피해야 할 대응/);
...
test('AI 결과에서 영문 내부 키와 확인되지 않은 직접 경험 표현을 제거한다',()=>{
  ...
  assert.match(edge,/analysisVersion='2026\.07\.21-teacher-action-json-safe-v5'/);
...
test('AI 분석은 마지막 분석 날짜·시간과 담임 실행 정보를 우선 표시한다',()=>{
  ...
  assert.match(app,/해석 → 관찰 → 코칭 순서/);
  assert.match(html,/해석·담임 관찰 포인트·코칭 방향을 먼저 제시/);
});
