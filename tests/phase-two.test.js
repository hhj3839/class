...
test('우리 반에서 보이는 모습은 내부 근거 꼬리표를 숨기고 실명과 운영 참고점을 표시한다',()=>{
...
test('AI 분석은 받은 평가 기반 최대 3명과 다음 대화·지원 방향을 만든다',()=>{
  ...
  assert.match(app,/담임이 살펴볼 점/);
  assert.match(app,/다음 대화와 지원/);
  assert.match(app,/<strong>담임 해석<\/strong>/);
  assert.match(app,/<strong>다음 대화와 지원<\/strong>/);
  assert.match(app,/학생과 나눌 질문/);
  assert.match(app,/피하면 좋은 대응/);
  assert.match(app,/필요할 때 응답 원문 확인/);
  assert.match(app,/연결된 응답 원문 보기/);
  assert.match(app,/function buildAiReportSection/);
  assert.match(app,/응답에서 확인된 내용/);
...
test('AI 분석은 전월 변화와 유형별 근거를 분리한다',()=>{
  ...
  assert.match(app,/이름 · 담임 해석 · 살펴볼 점 · 다음 대화와 지원/);
...
test('AI 화면과 PDF는 같은 담임용 용어를 직접 사용한다',()=>{
  ...
  for(const source of [visibleAi,pdfAi]){
    ...
    assert.match(source,/담임이 살펴볼 점/);
    assert.match(source,/다음 대화와 지원/);
    assert.match(source,/결과를 볼 때 참고할 점/);
...
test('학생 상세는 한눈에 보기에 응답 요약과 전월 변화를 포함하고 세 탭으로 자료를 구분한다',()=>{
  ...
  assert.match(app,/<h3>지난 응답과 비교<\/h3>/);
  ...
  assert.match(app,/>확인과 변화<\/button>/);
