...
function buildStudentSupportReportSection(student){
  ...
  return`<section class="pdf-ai-section"><div class="pdf-block pdf-section-heading"><h2>확인 기록 요약</h2><p>설문 ${counts.survey}건 · 확인이 필요한 응답 ${counts.signal}건 · 교사 확인 기록 ${counts.observation}건 · 확인 진행 중 ${open}건</p></div>${recent.length?recent.map(item=>`<article class="pdf-block pdf-support-item"><h4>${escapeHTML(String(item.date||'날짜 미정').slice(0,10))} · ${escapeHTML(item.label)}</h4><p><b>${escapeHTML(item.title)}</b><br>${escapeHTML(item.detail||'')}${item.note?`<br>${escapeHTML(item.note)}`:''}</p></article>`).join(''):'<p class="pdf-block">연결된 확인 기록이 없습니다.</p>'}</section>`
}
