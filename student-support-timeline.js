...
function studentReviewPanelHTML(student,month,supportSummary){
  ...
  return`<section class="panel student-attention-summary student-review-panel ${status==='reviewed'?'clear':'needs-attention'}"><div class="student-review-heading"><div><span>담임 확인</span><strong>${escapeHTML(label)}</strong></div><p>${escapeHTML(detail)}</p></div>${showControls?`<div class="student-review-controls" data-student-review-controls data-student-id="${escapeHTML(student.studentId||'')}" data-review-month="${escapeHTML(month)}"><label><span>이번 달 확인 결과</span><select data-student-review-status><option value="needs_review" ${status==='needs_review'?'selected':''}>이번 달 확인 필요</option><option value="reviewed" ${status==='reviewed'?'selected':''}>확인 완료</option><option value="carry_forward" ${status==='carry_forward'?'selected':''}>다음 설문에서 다시 보기</option><option value="rapid_followup" ${status==='rapid_followup'?'selected':''}>빠른 추가 확인</option></select></label><label class="student-review-focus"><span>살펴볼 내용</span><input data-student-review-focus maxlength="300" value="${escapeHTML(focus)}" placeholder="예: 모둠에서 의견을 말할 기회가 있는지"></label><label data-student-review-date-wrap ${status==='rapid_followup'?'':'hidden'}><span>빠른 확인 날짜</span><input type="date" data-student-review-date value="${escapeHTML(date)}"></label><button type="button" class="primary-button" data-save-student-review>담임 확인 저장</button></div>`:''}</section>`
}
...
document.addEventListener('change',event=>{
  const status=event.target.closest?.('[data-student-review-status]');if(!status)return;
  const controls=status.closest('[data-student-review-controls]'),dateWrap=controls?.querySelector('[data-student-review-date-wrap]');if(dateWrap)dateWrap.hidden=status.value!=='rapid_followup'
});
