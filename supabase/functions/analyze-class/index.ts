...
Deno.serve(async request=>{
  ...
  try{
    ...
    evidence.forEach(row=>{const values=received.get(row.student)||[];(row as any).received_relationships=values.length?{count:values.length,average:Number((values.reduce((sum,value)=>sum+value,0)/values.length).toFixed(2)),low_count:values.filter(value=>value<=2).length,high_count:values.filter(value=>value>=4).length,scores:values}:null});
    const schema={type:'object',additionalProperties:false,properties:{overall_judgment:{type:'string'},priority_students:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{student:{type:'string'},priority:{type:'string',enum:['즉시 확인','지속 관찰']},evidence:{type:'array',items:{type:'string'},maxItems:4},possible_interpretations:{type:'array',items:{type:'string'},maxItems:3},coaching_direction:{type:'string'},teacher_checks:{type:'array',items:{type:'string'},maxItems:3}},required:['student','priority','evidence','possible_interpretations','coaching_direction','teacher_checks']}},class_patterns:{type:'array',maxItems:4,items:{type:'object',additionalProperties:false,properties:{pattern:{type:'string'},evidence:{type:'array',items:{type:'string'},maxItems:4},teacher_check:{type:'string'}},required:['pattern','evidence','teacher_check']}},limitations:{type:'array',items:{type:'string'},maxItems:5}},required:['overall_judgment','priority_students','class_patterns','limitations']};
    const prompt=`당신은 초등학교 담임교사의 관찰을 돕는 보조 분석가입니다. ${month} 설문을 분석하세요. 학생별로 친구들에게 받은 관계 평가(received_relationships), 자기평가, 서술 응답을 함께 비교하세요. 응답 학생이 3명 이상이면 주의 깊게 볼 학생을 반드시 정확히 3명 선정하고, 입력 근거와 복수의 가능한 해석, 담임 관찰 포인트, 구체적인 코칭 방향을 제시하세요. 응답 학생이 3명 미만이면 가능한 인원만 선정하세요. 전체 결과를 아우르는 종합 판단도 작성하세요. 학생을 진단·낙인·확정하지 말고 입력에 직접 있는 근거만 쓰세요. 이름은 입력의 학생-N 표기를 그대로 유지하세요. 폭력·자해·즉각적 도움 요청은 즉시 확인으로 우선 표시하세요. 결석·미제출은 부정 신호로 해석하지 마세요. 관계 점수만으로 고립이나 집단을 확정하지 마세요. 가능한 해석은 복수 가설로, 교사 확인은 실제 관찰이나 비공개 대화 질문으로 제안하세요. JSON의 키와 학생-N 표기를 제외한 모든 자연어 값은 반드시 자연스럽고 이해하기 쉬운 한국어로 작성하세요. 영어 문장, 영어 제목, 영어 우선순위 표기는 사용하지 마세요.`;
