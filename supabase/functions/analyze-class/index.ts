...
const analysisVersion='2026.07.23-student-support-v13';
const relationshipAnalysisVersion='2026.07.23-relationship-coaching-v7';
...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(analysisType==='relationship'){
      ...
      const relationshipPrompt=`당신은 초등학교 담임교사가 월별 변화와 누적 친구 관계 응답을 학급 운영에 활용하도록 돕는 보조 분석가입니다. 입력은 현재 학급 명단에 있는 학생만 남긴 학생-N 익명 번호와 1~5점 관계 응답 계산 결과입니다. 결과는 학급 분석과 섞지 말고 관계 구조만 해석하세요. summary는 가장 중요한 우리 반 관계의 전체 모습을 80자 이내 한 문장으로 작성하세요. insights는 서로 겹치지 않는 핵심 관계 모습만 최대 2개 작성하세요. title은 짧은 명사형 제목으로 쓰고, observation에는 계산 결과로 확인되는 핵심 관계 모습을 80자 이내 한 문장으로 작성하세요. timeframe은 최신 월과 이전 월의 비교 근거가 있으면 '이번 달에 보이는 변화', 여러 달 반복된 누적 근거이면 '여러 달 이어진 관계 모습', 최신 월 응답이 부족하거나 비교할 이전 월이 없으면 '응답이 적어 추가 확인 필요' 중 하나를 선택하세요. 최신 월에서 연결이 확인되지 않았다는 이유만으로 관계가 끊겼다고 표현하지 마세요. teacher_check에는 가장 적절한 교실 장면 하나와 확인할 행동 하나를 합쳐 50자 이내 한 문장으로만 작성하세요. class_coaching에는 특정 학생을 드러내거나 관계 점수와 AI를 언급하지 않고 담임이 학급 전체에 실제로 건넬 수 있는 말 한 문장을 50자 이내로 작성하세요. 같은 뜻을 observation, teacher_check, class_coaching에 반복하지 마세요. 학급 운영 방법은 별도 항목으로 만들지 마세요. evidence는 반드시 [계산 결과]로 시작하고 핵심 수치 하나를 포함하며 최대 2개만 작성하세요. 학생을 인기·고립·문제 학생으로 단정하거나 관계 원인을 추측하지 마세요. 특정 학생은 반드시 '학생-N 학생은', '학생-N 학생과'처럼 익명 번호 뒤에 '학생'을 붙여 언급하세요. 모든 자연어는 이해하기 쉬운 한국어로 작성하고 여러 문장이나 번호 목록으로 나누지 마세요.`;
      const model='gpt-5.4-mini';
      const ai=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:AbortSignal.timeout(openAiTimeoutMs),headers:{Authorization:`Bearer ${openaiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model,reasoning:{effort:'low'},instructions:relationshipPrompt,input:JSON.stringify({months,latest_month:latestMonth,previous_month:previousMonth||null,latest_month_summary:latestSummary,previous_month_summary:previousMonth?previousSummary:null,cumulative:{relationship_response_count:relationshipRows.length,student_count:studentSummaries.length,mutual_high_relationships:mutual,students:studentSummaries}}),max_output_tokens:1800,text:{format:{type:'json_schema',name:'relationship_support_analysis',strict:true,schema:relationshipSchema}}})});
