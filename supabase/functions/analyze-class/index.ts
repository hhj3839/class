...
const analysisVersion='2026.07.21-teacher-action-json-safe-v5';
...
Deno.serve(async request=>{
  ...
  try{
    ...
    const schema={type:'object',additionalProperties:false,properties:{overall_judgment:{type:'string'},priority_students:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{student:{type:'string'},priority:{type:'string',enum:['즉시 확인','지속 관찰']},attention_reason:{type:'string'},protective_factors:{type:'array',items:{type:'string'},maxItems:3},observation_points:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{context:{type:'string'},behavior:{type:'string'},period:{type:'string'}},required:['context','behavior','period']}},coaching_directions:{type:'array',items:{type:'string'},maxItems:3},coaching_questions:{type:'array',items:{type:'string'},maxItems:4},avoid_actions:{type:'array',items:{type:'string'},maxItems:3},possible_interpretations:{type:'array',items:{type:'string'},maxItems:3},evidence:{type:'array',items:typedEvidence,maxItems:4},source_refs:{type:'array',items:{type:'string'},maxItems:4}},required:['student','priority','attention_reason','protective_factors','observation_points','coaching_directions','coaching_questions','avoid_actions','possible_interpretations','evidence','source_refs']}},class_patterns:{type:'array',maxItems:4,items:{type:'object',additionalProperties:false,properties:{pattern:{type:'string'},evidence:{type:'array',items:typedEvidence,maxItems:4},teacher_check:{type:'string'}},required:['pattern','evidence','teacher_check']}},limitations:{type:'array',items:{type:'string'},maxItems:5}},required:['overall_judgment','priority_students','class_patterns','limitations']};
    const prompt=`당신은 초등학교 담임교사의 관찰과 지원을 돕는 보조 분석가입니다. ${month} 설문을 분석하세요. 학생별로 친구들에게 받은 관계 평가, 자기평가, 서술 응답과 전월 변화를 함께 비교하며 단일 시점보다 반복·변화를 우선하세요. 의미 있는 복수 신호나 즉시 도움 요청이 있는 학생만 주의 깊게 볼 학생으로 최대 3명까지 선정하세요. 근거가 약하면 3명을 채우지 말고 빈 배열 또는 필요한 인원만 반환하세요. 학생 간 순위는 만들지 마세요. attention_reason은 담임이 왜 확인해야 하는지 2문장 이내로 요약하세요. protective_factors에는 현재 자료에서 확인되는 긍정적 요소만 쓰세요. observation_points는 실제 학교 장면을 중심으로 context에 언제·어디서, behavior에 판단 없이 볼 행동, period에 관찰 기간이나 횟수를 구체적으로 쓰세요. coaching_directions는 관찰 결과에 따라 적용할 수 있는 조건부 지원으로 작성하고 학생의 선택권을 존중하세요. coaching_questions는 학생을 비난하거나 답을 유도하지 않는 개방형 비공개 질문으로 작성하세요. avoid_actions에는 공개 지목, 즉시 화해 강요, 관계 점수 공개처럼 피해야 할 행동만 쓰세요. 가능한 해석은 서로 다른 복수 가설로 작성하세요. 모든 evidence 문장은 반드시 [계산 결과], [학생 원문], [친구 관찰] 중 하나로 시작하고 AI의 추측은 evidence에 넣지 마세요. 내부 JSON 필드명이나 영문 변수명을 결과에 절대 복사하지 말고 교사가 이해할 수 있는 한국어 항목명으로 바꾸세요. 특히 peer, hurt, needsHelp, unresolved, monthly_changes, responsibility, received_average_delta 같은 문자열을 출력하지 마세요. 관계 평균이나 비율을 근거로 쓸 때는 반드시 해당 응답자 수 또는 응답 건수를 함께 쓰세요. 현재 입력에는 직접 경험·직접 목격·전해 들음의 구분 정보가 없으므로 '직접 호소', '직접 경험', '직접 목격'이라고 확정하지 말고 '학생이 작성한 서술'이라고 표현하세요. 학생을 진단·낙인·확정하지 말고 입력에 직접 있는 근거만 쓰세요. 이름은 입력의 학생-N 표기를 그대로 유지하세요. 폭력·자해·즉각적 도움 요청은 즉시 확인으로 우선 표시하세요. 결석·미제출은 부정 신호로 해석하지 마세요. 관계 점수만으로 고립이나 집단을 확정하지 마세요. 학생-N 표기를 제외한 모든 자연어 값은 반드시 자연스럽고 이해하기 쉬운 한국어로 작성하세요. 영어 문장, 영어 제목, 영어 우선순위 표기는 사용하지 마세요.`;
