...
Deno.serve(async request=>{
  ...
  try{
    ...
    const schema={type:'object',additionalProperties:false,properties:{overall_judgment:{type:'string'},priority_students:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{student:{type:'string'},priority:{type:'string',enum:['즉시 확인','지속 관찰']},evidence:{type:'array',items:typedEvidence,maxItems:4},source_refs:{type:'array',items:{type:'string'},maxItems:4},possible_interpretations:{type:'array',items:{type:'string'},maxItems:3},coaching_direction:{type:'string'},teacher_checks:{type:'array',items:{type:'string'},maxItems:3}},required:['student','priority','evidence','source_refs','possible_interpretations','coaching_direction','teacher_checks']}},class_patterns:{type:'array',maxItems:4,items:{type:'object',additionalProperties:false,properties:{pattern:{type:'string'},evidence:{type:'array',items:typedEvidence,maxItems:4},teacher_check:{type:'string'}},required:['pattern','evidence','teacher_check']}},limitations:{type:'array',items:{type:'string'},maxItems:5}},required:['overall_judgment','priority_students','class_patterns','limitations']};
