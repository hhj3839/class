...
const analysisVersion='2026.07.23-student-support-v11';
const relationshipAnalysisVersion='2026.07.23-relationship-coaching-v5';
...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(analysisType==='relationship'){
      ...
      const relationshipRows=[...monthlyLatest.values()].filter((row:any)=>currentStudentNumbers.has(Number(row.student_number))).map((row:any)=>({...row,payload_json:{...(row.payload_json||{}),relationships:(row.payload_json?.relationships||[]).filter((item:any)=>currentStudentNumbers.has(Number(item.targetNumber)))}})).filter((row:any)=>(row.payload_json?.relationships||[]).length);
      ...
      const latestMonth=months.at(-1)||'',previousMonth=months.at(-2)||'',latestSummary=periodSummary(relationshipRows.filter((row:any)=>String(row.survey_month).slice(0,7)===latestMonth)),previousSummary=periodSummary(relationshipRows.filter((row:any)=>String(row.survey_month).slice(0,7)===previousMonth));
      const relationshipSchema={type:'object',additionalProperties:false,properties:{analysis_type:{type:'string',enum:['relationship']},summary:{type:'string'},insights:{type:'array',minItems:2,maxItems:3,items:{type:'object',additionalProperties:false,properties:{title:{type:'string'},timeframe:{type:'string',enum:['이번 달에 보이는 변화','여러 달 이어진 관계 모습','응답이 적어 추가 확인 필요']},observation:{type:'string'},teacher_check:{type:'string'},class_coaching:{type:'string'},operation_method:{type:'string'},evidence:{type:'array',minItems:1,maxItems:3,items:{type:'string'}}},required:['title','timeframe','observation','teacher_check','class_coaching','operation_method','evidence']}},limitations:{type:'array',maxItems:3,items:{type:'string'}}},required:['analysis_type','summary','insights','limitations']};
      const relationshipPrompt=`당신은 초등학교 담임교사가 월별 변화와 누적 친구 관계 응답을 학급 운영에 활용하도록 돕는 보조 분석가입니다. 입력은 현재 학급 명단에 있는 학생만 남긴 학생-N 익명 번호와 1~5점 관계 응답 계산 결과입니다. 결과는 학급 분석과 섞지 말고 관계 구조만 해석하세요. summary는 이번 달과 여러 달 흐름을 함께 고려한 우리 반 관계의 전체 모습을 2문장 이내로 작성하세요. insights는 근거가 강한 핵심 관계 모습만 2~3개 작성하세요. title은 학생 이름과 판단 표현을 피하고 카드 제목만 읽어도 관계 모습이 이해되는 완결된 한 문장으로 쓰세요. timeframe은 최신 월과 이전 월의 비교 근거가 있으면 '이번 달에 보이는 변화', 여러 달 반복된 누적 근거이면 '여러 달 이어진 관계 모습', 최신 월 응답이 부족하거나 비교할 이전 월이 없으면 '응답이 적어 추가 확인 필요' 중 하나를 선택하세요. 최신 월에서 연결이 확인되지 않았다는 이유만으로 관계가 끊겼다고 표현하지 마세요. observation에는 계산 결과로 확인되는 관계 모습을, teacher_check에는 쉬는 시간·모둠·놀이·협력 활동에서 담임이 확인할 장면을 작성하세요. class_coaching에는 특정 학생을 드러내거나 관계 점수와 AI를 언급하지 않고 담임이 학급 전체에 실제로 건넬 수 있는 말 한 문장을 60자 이내로 작성하세요. operation_method에는 자리·모둠·역할·협력 활동 등 실제 학급 운영 방법을 한 문장으로 작성하세요. evidence는 반드시 [계산 결과]로 시작하고 응답 인원·관계 수·반영 월 수 중 해당 수치를 포함하세요. 학생을 인기·고립·문제 학생으로 단정하거나 관계 원인을 추측하지 마세요. 고정된 친구 집단이라고 확정하지 말고 담임 관찰과 학생 대화가 필요함을 유지하세요. 특정 학생은 반드시 '학생-N 학생은', '학생-N 학생과'처럼 익명 번호 뒤에 '학생'을 붙여 언급하고, '학생-N은', '학생-N과'처럼 번호에 조사를 직접 붙이지 마세요. 모든 자연어는 이해하기 쉬운 한국어로 작성하고 각 문장은 120자 이내로 제한하세요.`;
      const model='gpt-5.4-mini';
      const ai=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:AbortSignal.timeout(openAiTimeoutMs),headers:{Authorization:`Bearer ${openaiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model,reasoning:{effort:'low'},instructions:relationshipPrompt,input:JSON.stringify({months,latest_month:latestMonth,previous_month:previousMonth||null,latest_month_summary:latestSummary,previous_month_summary:previousMonth?previousSummary:null,cumulative:{relationship_response_count:relationshipRows.length,student_count:studentSummaries.length,mutual_high_relationships:mutual,students:studentSummaries}}),max_output_tokens:2400,text:{format:{type:'json_schema',name:'relationship_support_analysis',strict:true,schema:relationshipSchema}}})});
    ...
    const schema={type:'object',additionalProperties:false,properties:{overall_judgment:{type:'string'},priority_students:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{student:{type:'string'},priority:{type:'string',enum:['즉시 확인','지속 관찰']},attention_reason:{type:'string'},protective_factors:{type:'array',items:{type:'string'},maxItems:2},observation_points:{type:'array',maxItems:2,items:{type:'object',additionalProperties:false,properties:{context:{type:'string'},behavior:{type:'string'},period:{type:'string'}},required:['context','behavior','period']}},coaching_directions:{type:'array',items:{type:'string'},maxItems:2},coaching_questions:{type:'array',items:{type:'string'},maxItems:2},avoid_actions:{type:'array',items:{type:'string'},maxItems:2},possible_interpretations:{type:'array',items:{type:'string'},maxItems:2},evidence:{type:'array',items:typedEvidence,maxItems:4},source_refs:{type:'array',items:{type:'string'},maxItems:4}},required:['student','priority','attention_reason','protective_factors','observation_points','coaching_directions','coaching_questions','avoid_actions','possible_interpretations','evidence','source_refs']}},class_patterns:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,properties:{pattern:{type:'string'},evidence:{type:'array',items:typedEvidence,maxItems:3},teacher_check:{type:'string'}},required:['pattern','evidence','teacher_check']}},limitations:{type:'array',items:{type:'string'},maxItems:5}},required:['overall_judgment','priority_students','class_patterns','limitations']};
    const prompt=`당신은 초등학교 담임교사의 관찰과 지원을 돕는 보조 분석가입니다. ${month} 설문을 분석하세요. 학생별로 친구들에게 받은 관계 평가, 자기평가, 서술 응답과 전월 변화를 함께 비교하며 단일 시점보다 반복·변화를 우선하세요. 의미 있는 복수 신호나 즉시 도움 요청이 있는 학생만 주의 깊게 볼 학생으로 최대 3명까지 선정하세요. 근거가 약하면 3명을 채우지 말고 빈 배열 또는 필요한 인원만 반환하세요. 학생 간 순위는 만들지 마세요. 결과는 담임 해석, 관찰 포인트, 코칭의 세 단계로 바로 사용할 수 있게 작성하세요. attention_reason은 담임이 왜 확인해야 하는지 2문장 이내로 요약하세요. protective_factors에는 현재 자료에서 명확히 확인되는 긍정적 요소만 최대 2개 쓰고 근거가 없으면 빈 배열을 반환하세요. observation_points는 최대 2개로 제한하고 실제 학교 장면을 중심으로 context에 언제·어디서, behavior에 판단 없이 볼 행동, period에 관찰 기간이나 횟수를 구체적으로 쓰세요. coaching_directions는 최대 2개로 제한하며 관찰 결과에 따라 적용할 수 있는 조건부 지원으로 작성하고 학생의 선택권을 존중하세요. coaching_questions는 최대 2개로 제한하고 학생을 비난하거나 답을 유도하지 않는 개방형 비공개 질문으로 작성하세요. avoid_actions는 해당 학생에게 특별히 주의할 대응이 있을 때만 최대 2개 쓰고, 일반적인 주의사항이거나 특별한 위험이 없으면 빈 배열을 반환하세요. 가능한 해석은 검증용 데이터로만 최대 2개 작성하고 기본 화면에는 표시되지 않습니다. class_patterns는 여러 학생의 응답에서 공통으로 확인되는 우리 학급의 구체적인 모습만 2~3개 작성하세요. pattern은 '일부 학생', '관계 변화' 같은 개괄적 제목 대신 인원·영역·방향을 포함한 완결된 한 문장으로 쓰세요. 특정 학생의 응답이 흐름을 이해하는 데 꼭 필요하면 pattern이나 evidence에 학생-N을 최대 3명까지 표시하세요. evidence에는 그 문장을 바로 이해할 수 있는 학급 수치나 익명 학생 근거를 최대 3개 넣고, teacher_check에는 담임이 교실에서 살펴볼 구체적인 장면을 한 문장으로 쓰세요. 모든 evidence 문장은 반드시 [계산 결과], [학생 원문], [친구 관찰] 중 하나로 시작하고 AI의 추측은 evidence에 넣지 마세요. 내부 JSON 필드명이나 영문 변수명을 결과에 절대 복사하지 말고 교사가 이해할 수 있는 한국어 항목명으로 바꾸세요. 특히 peer, hurt, needsHelp, unresolved, monthly_changes, responsibility, received_average_delta 같은 문자열을 출력하지 마세요. 관계 평균이나 비율을 근거로 쓸 때는 반드시 해당 응답자 수 또는 응답 건수를 함께 쓰세요. 현재 입력에는 직접 경험·직접 목격·전해 들음의 구분 정보가 없으므로 '직접 호소', '직접 경험', '직접 목격'이라고 확정하지 말고 '학생이 작성한 서술'이라고 표현하세요. 학생을 진단·낙인·확정하지 말고 입력에 직접 있는 근거만 쓰세요. 이름은 입력의 학생-N 표기를 그대로 유지하세요. 폭력·자해·즉각적 도움 요청은 즉시 확인으로 우선 표시하세요. 결석·미제출은 부정 신호로 해석하지 마세요. 관계 점수만으로 고립이나 집단을 확정하지 마세요. 학생-N 표기를 제외한 모든 자연어 값은 반드시 자연스럽고 이해하기 쉬운 한국어로 작성하세요. 영어 문장, 영어 제목, 영어 우선순위 표기는 사용하지 마세요.`;
    const classCoachingInstruction=' priority_students의 coaching_questions는 담임이 학생에게 비공개로 실제 건넬 수 있는 말 한 문장을 60자 이내로 작성하고, coaching_directions는 그 대화 뒤의 조건부 지원 방향으로 분리하세요. class_patterns는 우리 반 응답 흐름입니다. 자기평가·도움 요청·공통 고민 등 학급 전체 응답의 인원·평균·변화를 중심으로 쓰고, 흐름을 이해하는 데 꼭 필요한 학생만 학생-N으로 최대 3명까지 표시하세요. 관계망·자리·모둠·친구 집단 해석은 넣지 마세요. teacher_check에는 담임이 교실에서 살펴볼 구체적인 장면을 한 문장으로 작성하세요.';
