export const VERSION='2026.09.09-student-coaching-v1';
...
export function buildEvidence(context,normalizer){
  ...
  for(const row of (context.observations||[]).slice().sort((a,b)=>String(b.updated_at).localeCompare(String(a.updated_at))).slice(0,6)){
    const month=String(row.survey_month||'').slice(0,7);if(transfer&&month>transfer.slice(0,7))continue;
    add('교사 관찰',month,'교사가 기록한 관찰 사실',row.observed_fact,{kind:'observation',observationId:row.id,field:'observed_fact'});
    add('교사 면담 기록',month,'교사가 기록한 면담 내용',row.interview_note,{kind:'observation',observationId:row.id,field:'interview_note'});
  }
  ...
  const hasNarrative=sources.some(source=>['학생 응답','교사 관찰','교사 면담 기록'].includes(source.type));
